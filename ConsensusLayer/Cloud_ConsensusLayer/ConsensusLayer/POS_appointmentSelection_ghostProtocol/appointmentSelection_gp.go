...
type Config struct {
	...
	NextValidator  string    `json:"NextValidator"`
	FirstValidator string    `json:"FirstValidator"`
}
...
//based on the strategy to settle the bootnode address
...
func main() {
	...
	genesisBlock = Block{0, t, tr, "Genesis transaction: 06/10/2020", calculateBlockHash(genesisBlock), "", firstValidator}
...
func readConfig() {
	...
	appointedValidator = config.NextValidator
	firstValidator = config.FirstValidator
	...
	*/
	...
	case messageType.Bootstrap:
		...
		for _, peer := range peerList {
			err := g.AddPeer(peer)
			if err == nil {
				msg := message.JSONMessage{"NewJoinNode", g.PubAddr, time.Now().String(), address, 0}
