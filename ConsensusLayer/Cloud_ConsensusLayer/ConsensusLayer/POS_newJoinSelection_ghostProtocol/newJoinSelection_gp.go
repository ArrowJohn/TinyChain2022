...
//var messageType = message.MessageType{"NewJoinNode", "Bootstrap", "TEST", "CurrentWinner", "NewTransaction", "NewProposedBlock", "LatestBlockChain"}
...
func main() {
	...
	transaction = Transaction{t, "genesisTransaction", "0", calculateHash(t + "genesisTransaction" + "0" + "Hao" + string(20)), Addresses[0], 500}

	genesisBlock = Block{0, t, transaction, "Genesis transaction: 06/10/2020", calculateBlockHash(genesisBlock), "", "8891"}
		...
		for i, n := range NodesList {
			if n == address {
				if i != len(NodesList)-1 {
					fmt.Println(NodesList[i+1])
					g.AddPeer("localhost:" + NodesList[i+1])
					break
				} else {
					fmt.Println(NodesList[0])
	...
	*/
	...
	g = gossip.NewGossip(PublicAdd, PrivateAdd, Port, 5)
	...
	go func() {
		for {
			if len(validators) == 3 {
				currentValidator = validators[0]
				stdout <- "first validator has been settled."
				break
			}
		}
	}()
...
func pickWinner() {
	...
	if currentValidator == address {
		...
		if len(transactions) > 0 {
			...
			currentValidator = getNextValidator(currentValidator)
...
func getNextValidator(current string) string {
	for i, item := range validators {
		if item == current && i != (len(validators)-1) {
...
func marshalToJSON(msg message.JSONMessage) string {
...
// FindTransFromBlockchain FindTrans takes a slice and looks for an element in it. If found it will
// return it's key, otherwise it will return -1 and a bool of false.
...
func msgHandler(msg message.JSONMessage) {
	...
	case messageType.NewProposedBlock:
		...
		//broadcastBlockchain(string(msg.Body), messageType.NewProposedBlock)

	case messageType.NewJoinNode:
		stdout <- "[gossip] Received a NewJoinNode message(add):" + msg.Body
		split := strings.Split(msg.Body, ":")
		if len(split) == 2 {
			_, found := Find(validators, split[0])
			if !found {
				validators = append(validators, split[0])
	...
	*/
	...
	case messageType.Bootstrap:
		...
		for _, peer := range peerList {
			...
			err := g.AddPeer(peer)
			if err == nil {
				msg := message.JSONMessage{"NewJoinNode", g.PubAddr, time.Now().String(), g.PubAddr, 0}
